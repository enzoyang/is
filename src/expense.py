...
import web
import user
from common import generalIdentity, generateWorkbook
from account import AuthBase
from settings import expense_render, user_render, manager_render, Errors, Infos
...
class IndexManager(AuthBase, AccountBase):
    def __init__(self):
        AuthBase.__init__(self)
        AccountBase.__init__(self)
        d['links'] = [('/expense/', u'返回水电管理系统首页'), ('/expense/manager/generate/', u'生成登记表'), ]
    
    def GET(self):
        d['totalUser'] = web.ctx.orm.query(User).all()
        return manager_render.index(**d)

class Generate(AuthBase, AccountBase):
    def __init__(self):
        AuthBase.__init__(self)
        AccountBase.__init__(self)
        
    def GET(self):
        path = generateWorkbook()
        return path
